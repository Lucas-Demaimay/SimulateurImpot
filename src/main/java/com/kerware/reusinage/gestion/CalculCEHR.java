...
public class CalculCEHR extends StategieCalcul {
    ...
    public double calcul() {
        ...
        for(TranchesCEHR tranche : TranchesCEHR.values()){
            if(revenuFiscalReference >= tranche.getValeurDebutDeTranche() && revenuFiscalReference <= tranche.getValeurFinDeTranche()){
                if(nbPart == 1){
                    contributionExceptionelle += ( revenuFiscalReference - tranche.getValeurDebutDeTranche() ) * tranche.getTauxCEHRCelibataire();
                } else {
                    contributionExceptionelle += ( revenuFiscalReference - tranche.getValeurDebutDeTranche() ) * tranche.getTauxCEHRCouple();
                }
            }
        }
